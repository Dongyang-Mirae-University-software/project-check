# 실행 상태

- 스캔 시각: 2026-06-05 14:27:32 KST

| 프로젝트 | 분류 | 실행 상태 | PM2 | Docker | 포트 |
| --- | --- | --- | --- | --- | --- |
| AiSilverBridgeSky | AI 서버 | 확인 불가 | 확인 불가 | 확인 불가 | 3099, 4306, 4747, 8000, 14122 |
| ChatSilverBridge | AI 서버 | 실행 중 | 확인 불가 | chatsilverbridge-api-1, chatsilverbridge-mysql-1, chatsilverbridgetest-web-1 | 8010, 8090 |
| ChatSilverBridgeTest | 프론트엔드 | 실행 중 | 확인 불가 | chatsilverbridgetest-web-1 | 8010 |
| Playground | 기타 | 확인 불가 | 확인 불가 | 확인 불가 | 확인 불가 |
| project-check | 기타 | 실행 중 | project-check | 확인 불가 | 2026, 3307, 6012, 6015, 6516, 11000, 11001, 11002, 11003, 11004, 26510, 59905 |
| SilverBridgeAi | AI 서버 | 실행 중 | 확인 불가 | silverbridge-ai-server | 1000, 1234, 1280, 1500, 2026, 2700, 8080, 8456 |
| SilverBridgeAiServer | AI 서버 | 실행 중 | 확인 불가 | silverbridge-ai-server | 1008, 5432, 6012, 6015, 6017, 6019 |
| SilverBridgeBe | 백엔드 | 실행 중 | 확인 불가 | 확인 불가 | 3000, 5173, 5432, 6379, 6511, 8080, 18000, 60480 |
| SilverBridgeFe | 프론트엔드 | 실행 중 | 확인 불가 | 확인 불가 | 3000, 5000, 6000, 60480, 60836 |
| SilverBridgeReservation | 백엔드 | 실행 중 | 확인 불가 | silverbridgereservation-reservation-api-1 | 5173, 6015, 6017, 6018 |
| SilverBridgeSky | 백엔드 | 실행 중 | project-check | 확인 불가 | 3000, 5432, 6379, 6501, 8000, 8080 |
| SilverBridgeSSOBe | 백엔드 | 실행 중 | 확인 불가 | 확인 불가 | 3306, 6379, 6501, 18000, 60480 |
| SilverBridgeSSOFe | 프론트엔드 | 확인 불가 | 확인 불가 | 확인 불가 | 확인 불가 |
| SilverBridgeStreamTestFe | 프론트엔드 | 실행 중 | 확인 불가 | 확인 불가 | 1012, 6018 |
| Trump | 백엔드 | 실행 중 | 확인 불가 | trump-pulse-web, trump-pulse-api, trump_news_server, trump-pulse-redis, trump-pulse-postgres | 3000, 4000, 5000, 5432, 6379, 11001, 11002, 11004, 60000 |
| VisionX-DMU | 백엔드 | 정지 | 확인 불가 | 확인 불가 | 확인 불가 |
| WhitehouseFE | 프론트엔드 | 정지 | 확인 불가 | 확인 불가 | 1280, 3000 |

## 요약

- AiSilverBridgeSky: 정지 또는 확인 불가
- ChatSilverBridge: 실행 중 / PM2 없음 / Docker 3개 / 포트 확인 불가
- ChatSilverBridgeTest: 실행 중 / PM2 없음 / Docker 1개 / 포트 확인 불가
- Playground: 정지 또는 확인 불가
- project-check: 실행 중 / PM2 1개 / Docker 없음 / 포트 3307, 6012, 6015, 6516, 11000, 11001, 11002, 11003, 11004
- SilverBridgeAi: 실행 중 / PM2 없음 / Docker 1개 / 포트 확인 불가
- SilverBridgeAiServer: 실행 중 / PM2 없음 / Docker 1개 / 포트 6012, 6015, 6017, 6019
- SilverBridgeBe: 실행 중 / PM2 없음 / Docker 없음 / 포트 6379, 6511
- SilverBridgeFe: 실행 중 / PM2 없음 / Docker 없음 / 포트 6000
- SilverBridgeReservation: 실행 중 / PM2 없음 / Docker 1개 / 포트 6015, 6017, 6018
- SilverBridgeSky: 실행 중 / PM2 1개 / Docker 없음 / 포트 6379
- SilverBridgeSSOBe: 실행 중 / PM2 없음 / Docker 없음 / 포트 3306, 6379
- SilverBridgeSSOFe: 정지 또는 확인 불가
- SilverBridgeStreamTestFe: 실행 중 / PM2 없음 / Docker 없음 / 포트 6018
- Trump: 실행 중 / PM2 없음 / Docker 5개 / 포트 6379, 11001, 11002, 11004
- VisionX-DMU: 정지 또는 확인 불가
- WhitehouseFE: 정지 또는 확인 불가